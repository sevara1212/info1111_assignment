...
export default function AdminLogin() {
  ...
  // Redirect or block access after login
  useEffect(() => {
    if (!authLoading) {
      if (user && userData?.role === 'admin') {
        ...
        signOut(); // optional: log them out if unauthorized
      }
    }
  ...
  if (authLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        ...
        <span className="ml-4 text-lg text-gray-700">Checking credentials...</span>
      </div>
    );
  }
      ...
      <div className="w-full max-w-md bg-white rounded-lg shadow-lg p-8">
        ...
        <div className="mt-6 text-center">
          <p className="text-gray-600">
            ...
            <Link href="/auth/signup/admin" className="text-green-600 hover:text-green-800 font-medium">
              Create Admin Account
            </Link>
          </p>
          <Link href="/" className="text-sm text-gray-600 hover:text-gray-900 mt-4 inline-block">
            Back to Home
          </Link>
        </div>
      </div>
